...
export class BuildingsService {
  ...
  public saveBuilding(aBuilding) {
    //SOLO SE GUARDARA EN EL CASO DE QUE SE CREE UN NUEVO WORKSPACE, SI EXISTÍA, LO PISA.
    return new Promise<boolean>((resPromesa) => {
      this.angularfirebaseDB.database
        .ref(this.tabla + aBuilding.buildingIdentifier)
        .set(aBuilding)
        .then(
          (resolve) => {
            resPromesa(true);
  ...
  public getBuildingForWorkspace(aBuildingIdentifier) {
    return new Promise<Building>((resPromesa) => {
      var buildingReference = this.angularfirebaseDB.database.ref(
        this.tabla + aBuildingIdentifier
      );
      ...
      cordova.plugins.Situm.fetchBuildings(
        (res) => {
          ...
          this.buildings.forEach(
            //A CADA EDIFICIO PIDO SUS PISOS
            (edificio) => {
              this.fetchFloorsFromBuilding(edificio).then(
                (res) => {
                ...
                (error) => {
                  ...
                  console.log(error);
                  const errorMsg =
                    'Hubo un error al cargar los pisos, intente nuevamente.';
        ...
        (error) => {
          const errorMsg =
            'Hubo un error al cargar los edificios, intente nuevamente.';
          this.presentToast(errorMsg, 'top', null);
        }
      );
    });
